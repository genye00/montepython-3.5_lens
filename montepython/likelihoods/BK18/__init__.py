...
class BK18(Likelihood_sn):
    ...
    def __init__(self, path, data, command_line):
        ...
        try:
            ...
            super(BK18, self).__init__(path, data, command_line)
        except IOError:
            raise io_mp.LikelihoodError(
                "The BK18 data files were not found. Please download the "
                "data from http://bicepkeck.org"
                ", extract it, and copy the folders inside"
                "`BK18_cosmomc/data/` to `your_montepython/data/BK18/`")
        ...
        arguments = {
            ...
            'modes': 's, t',
            'l_max_scalars': 2700,
            'non_linear':'HALOFIT' if self.do_nonlinear else '',
            'l_max_tensors': self.cl_lmax}
        if hasattr(self, 'k_max_tau0_over_l_max'):
            arguments.update({'k_max_tau0_over_l_max': self.k_max_tau0_over_l_max})
        else:
            arguments.update({'k_max_tau0_over_l_max': 3.0})
        if hasattr(self, 'accurate_lensing'):
            arguments.update({'accurate_lensing': self.accurate_lensing})
        else:
            arguments.update({'accurate_lensing': 1})
        ...
        for k in range(nbins):
            ...
            #if len(tmp)>self.cl_lmax:
            #    print("Warning: Windowfile %s (%d lines) contains additional lines outside the l range lmax=%d"%(windowfile,len(tmp),int(self.cl_lmax)))
            # Apply mask
            tmp = tmp[:,mask]
    ...
    def loglkl(self, cosmo, data):
        ...
        for k in range(nbins):
            ...
            # Hard cut by bins, Gen Ye
            if k+1<int(self.use_bins[0]) or k+1>int(self.use_bins[1]):
                X[k*ncrossmaps:(k+1)*ncrossmaps] = 0
            else:
